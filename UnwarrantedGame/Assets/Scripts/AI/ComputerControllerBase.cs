using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.Tilemaps;

public class ComputerControllerBase : MonoBehaviour {
	//Returns the basic direction required for target 1 to reach target 2
	public Vector3 moveTowardSimple(GameObject target1, GameObject target2){
		Vector3Int currentTile1 = new Vector3Int((int)target1.transform.position.x, (int)target1.transform.position.z, 0);
		Vector3Int currentTile2 = new Vector3Int((int)target2.transform.position.x, (int)target2.transform.position.z, 0);
		int xDiff = currentTile1.x - currentTile2.x;
		int yDiff = currentTile1.y - currentTile2.y;
		if (xDiff > yDiff){
			if(xDiff > 0)
				return Vector3.left;
			else
				return Vector3.right;
		} else if (yDiff > xDiff) {
			if(yDiff > 0)
				return Vector3.back;
			else
				return Vector3.forward;
		} else { //even values
			if(currentTile1.y - currentTile2.y > 0)
				return Vector3.back;
			else if (currentTile1.y - currentTile2.y < 0)
				return Vector3.forward;
			else if (currentTile1.x - currentTile2.x > 0)
				return Vector3.left;
			else if (currentTile1.x - currentTile2.x < 0)
				return Vector3.right;
		}
		//UnityEngine.Tilemaps.Tilemap tilemap = GameObject.Find("Grid").GetComponent<Grid>().GetComponentInChildren<Tilemap>();
		return Vector3.zero;
	}
}
